...
export class SignupComponent implements OnInit {
  ...
  // detecting scroll to toggle navbar stuff
  ...
  signUpFunction(f: any) {
    let tempString = (f.controls.userName.value).substr((f.controls.userName.value).length - 6, (f.controls.userName.value).length - 1);
    if (tempString === '-admin') {
